...
let get_game2 (input: string list) : race = 
  ...
  let dist = input
             ...
             |> List.hd
             |> get_value2 in
  {time = time; distance_record = dist}
;;

(* logic *)


let get_distance (prep: int) (time: int) : int =
  prep * (time - prep)
;;

let is_above (race:race) (prep: int) : bool =
  if (prep < 0 ||  prep > race.time) then 
    false
  else
