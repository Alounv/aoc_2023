...
type card = { id : int; winning : int list; owned : int list }
...
let card_win (card : card) : int =
  let rec count = function
    | [] -> 0
    | x :: rest ->
        if List.mem x card.winning then 1 + count rest else count rest
  in
  count card.owned
...
let get_won_cards (id : int) (win : int) : int list =
  let rec loop (id : int) (win : int) (acc : int list) : int list =
    if win = 0 then acc else loop (id + 1) (win - 1) (id :: acc)
  in
  loop (id + 1) win []

module M = Map.Make (Int)

let increment_value (count_map : int M.t) (cardId : int) (increment : int) :
    int M.t =
...
let rec increment_card_values (count_map : int M.t) (cardsIds : int list)
    (increment : int) : int M.t =
  match cardsIds with
  | [] -> count_map
  | cardId :: rest ->
...
let rec get_scratched_card (wins_map : int M.t) (count_map : int M.t)
    (index : int) : int M.t =
...
let logic2 (lines : string list) : int =
  ...
  let wins_map =
    List.fold_left (fun acc (id, win) -> M.add id win acc) M.empty cards_wins
  in
