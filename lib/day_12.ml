...
let get_possible_starts (line : char array) (size : int) (prev_group : group)
    (prev_group_end : int) (len : int) (tl : int list) : int list =
  let min_start = if prev_group.size = 0 then 0 else prev_group_end + 2 in
  (* + 2 because we need at least one dot between groups *)
  let max_end = len - (tl |> List.fold_left (fun acc s -> acc + s + 1) 0) in
  ...
  let rec recursive (groups_sizes : int list) (prev_path : path) : int =
    ...
    if not is_valid then 0
      ...
      | None -> (
          ...
          | size :: tl ->
            let possible_starts =
              get_possible_starts line size prev_group prev_group_end len tl
            in
